...
- [Audyt całościowy kodu](CODEBASE-AUDIT-2026-07-19.md) — 41 ustaleń + rejestr statusu integracji.
...
- Awaria fallbacku PaddleOCR nie unieważnia już ukończonego zadania OCR: `ExtractWithFallbackAsync` owija wywołanie fallbacku w `try/catch`, po pustym wyniku Tesseracta zwraca pusty wynik prymarny (`completed`, nie `failed`) i rejestruje diagnostykę przez `Trace`; anulowanie inicjowane przez użytkownika nadal propaguje.
- OCR Tesseracta zachowuje polskie znaki: `TesseractOcrEngine.StartProcess` ustawia `StandardOutputEncoding`/`StandardErrorEncoding = UTF8`, więc tekst UTF-8 nie degraduje się do mojibake w `content_segments` (analogicznie do `PaddleOcr.cs` i `MediaTranscription.cs`).
